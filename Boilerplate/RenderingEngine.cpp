...
void RenderingEngine::RenderScene(const std::vector<Geometry>& objects, GLFWwindow* window, glm::mat4 m_v_p, SceneNode* sceneGraph) {
	...
	int counter = 0;
	glm::mat4 sunTrans = SceneNode::traverseGraph(sceneGraph) * sceneGraph->getLocalTransformation();
	GLuint sunID = glGetUniformLocation(shaderProgram, "sunMVP");	// sun
	glUniformMatrix4fv(sunID, 1, GL_FALSE, glm::value_ptr(sunTrans));

	std::queue<SceneNode*> queue;
	queue.push(sceneGraph);
	glm::mat4 m_v = m_v_p;
	while (!queue.empty()) {
		glUseProgram(shaderProgram);

		SceneNode* currentNode = queue.front();
		queue.pop();
		m_v_p = m_v * SceneNode::traverseGraph(currentNode) * currentNode->getLocalTransformation();
		GLuint MatrixID = glGetUniformLocation(shaderProgram, "MVP");	// sun
		glUniformMatrix4fv(MatrixID, 1, GL_FALSE, glm::value_ptr(m_v_p));
		...
		glBindTexture(GL_TEXTURE_2D, currentNode->mySphere->my_texture.textureID);
		...
		GLuint uniformLocation2 = glGetUniformLocation(shaderProgram, "diffuse");
		glUniform1i(uniformLocation2, currentNode->mySphere->diffuse);

		glBindVertexArray(currentNode->mySphere->vao);
		//std::cout <<currentNode->mySphere->verts.size() << "\n";
		glDrawArrays(currentNode->mySphere->drawMode, 0, currentNode->mySphere->verts.size());

		// reset state to default (no shader or geometry bound)
		glBindVertexArray(0);

		for (SceneNode* aChild : currentNode->children) {
			queue.push(aChild);
		}
